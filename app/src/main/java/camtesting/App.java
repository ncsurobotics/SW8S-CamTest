...
public class App {
    ...
    static String h264encPipeline(int bitrate){
        ...
        if(new File("/etc/nv_tegra_release").exists()){
            ...
            return String.format("omxh264enc bitrate=%d control-rate=variable ! video/x-h264,profile=baseline", bitrate);
        }else{
            // No clue what system this is. Fallback on software encoder.
            return String.format("x264enc tune=zerolatency speed-preset=ultrafast bitrate=%d ! video/x-h264,profile=baseline", bitrate);
        }
    }
